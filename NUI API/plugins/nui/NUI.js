...
NUI.NUIArtTips = NUI.NUIArtSubmit = function(options){
	var options = options||{},
		_options = {};
	_options.fixed = true;
...
NUI.NUIArtIframe = function(options){
	var options = options||{},
		_options = {};
...
$(function(){
    ...
    Table.prototype = {
        ...
        // 全选
        check: function($ele){
            $(document).delegate('.SysCheckAll', 'click', function(){
                $eles = $(this).parents('table').eq(0).find('tbody input:checkbox');
            ...
            $(document).delegate('tbody input:checkbox', 'click', function(){
                var $checkAll = $(this).parents('table').find('.SysCheckAll'),
                    $eles = $(this).parents('table').eq(0).find('tbody input:checkbox'),
                    len = $eles.length;
                if($(this).prop('checked')){
    ...
    var $tab = $('.NUITable');
    if($tab.length){
        var oTable = new Table($tab);
    }
	var $tabAjax = $('.NUITableAjax');
	if($tabAjax.length){
		function TableAjax($ele){
			this.ele = $ele;
			this.options = {
					...
					for(var j=0,len=th.length;j<len;j++){
						...
						if(j==0&&v.find('input[type="checkbox"]').length){							
                            ...
                            try{
                                ...
                                throw new Error(format+' is not defined or '+format+' is not a function');
                            }
						}
						else{
		...
		var table = new TableAjax($tabAjax);
	}
});
/**
